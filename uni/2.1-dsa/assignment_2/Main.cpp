...
vector<vector<float>> inverse(vector<vector<int>> matrix) {
    int det_all = det(matrix);
    int dim = matrix.size();
    auto res = vector<vector<float>>(dim, vector<float>(dim, 0));

    for (int i = 0; i < dim; ++i) {
        for (int j = 0; j < dim; ++j) {
            auto cur_det = det(get_reduced(matrix, i, j, dim));
            auto sign = (i + j) % 2 ? -1 : 1;
            res[j][i] = (float) (cur_det * sign) / (float) det_all;
        }
    }
...
template <typename T> void print_vector(vector<vector<T>> v) {
...
//    cout << det(mat) << '\n';
    ...
    auto inverse_m = inverse(mat);
    print_vector(inverse_m);
    return 0;
}